...
UserManager::UserManager(const std::string& data_dir, const UserInfo& root)
    : data_dir_(data_dir) {
...
Status UserManager::Login(const std::string& name, const std::string& password,
                          const std::string& uuid) {
  MutexLock lock(&mu_);
  std::map<std::string, UserInfo>::iterator user_it = user_list_.find(name);
  if (user_it == user_list_.end()) {
    LOG(WARNING) << "Not exist user tried to login: " << name;
    return kUnknownUser;
  }
...
Status UserManager::Logout(const std::string& uuid) {
  ...
  if (online_it == logged_users_.end()) {
    LOG(WARNING) << "Logout for an not exist user: " << uuid;
    return kUnknownUser;
  }
...
Status UserManager::Register(const std::string& name,
...
Status UserManager::ForceOffline(const std::string& myid,
                                 const std::string& name) {
  MutexLock lock(&mu_);
  std::map<std::string, std::string>::const_iterator online_it =
      logged_users_.find(myid);
  if (online_it == logged_users_.end()) {
    return kUnknownUser;
  }
  std::map<std::string, UserInfo>::const_iterator user_it =
      user_list_.find(name);
  if (user_it == user_list_.end()) {
    return kUnknownUser;
  }
...
Status UserManager::DeleteUser(const std::string& myid,
                               const std::string& name) {
  MutexLock lock(&mu_);
  std::map<std::string, std::string>::const_iterator online_it =
      logged_users_.find(myid);
  ...
  std::map<std::string, UserInfo>::iterator user_it = user_list_.find(name);
  if (user_it == user_list_.end()) {
    LOG(WARNING) << "Try to delete an not exist user: " << name;
    return kNotFound;
  }
...
bool UserManager::WriteToDatabase(const UserInfo& user) {
  ...
  leveldb::Status status =
      user_db_->Put(leveldb::WriteOptions(), user.username(), user.passwd());
  return status.ok();
}

bool UserManager::WriteToDatabase(const std::string& name,
                                  const std::string& password) {
  leveldb::Status status =
      user_db_->Put(leveldb::WriteOptions(), name, password);
  return status.ok();
}
