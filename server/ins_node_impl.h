...
struct Session {
  ...
  Session(const std::string& sid, const std::string& uid)
      : session_id(sid), uuid(uid), last_report_time(0) {}
};

typedef multi_index_container<
    Session,
    indexed_by<
        hashed_unique<member<Session, std::string, &Session::session_id> >,
        ordered_non_unique<
...
struct WatchAck {
  ...
  google::protobuf::Closure* done;
  WatchAck(WatchResponse* rsps, google::protobuf::Closure* dn)
      : response(rsps), done(dn) {}
  ~WatchAck() {
    if (done) {
      done->Run();
    }
...
typedef multi_index_container<
    WatchEvent,
    indexed_by<
        ordered_non_unique<member<WatchEvent, std::string, &WatchEvent::key> >,
        ordered_non_unique<
...
class InsNodeImpl : public InsNode {
 ...
 private:
  ...
  void ParseValue(const std::string& value, LogOperation& op,
  ...
  bool TriggerEvent(const std::string& watch_key, const std::string& key,
                    const std::string& value, bool deleted);
  void TriggerEventWithParent(const std::string& key, const std::string& value,
                              bool deleted);
  ...
  bool LockIsAvailable(const std::string& user, const std::string& key,
