...
void InsNodeImpl::VoteCallback(const ::galaxy::ins::VoteRequest* request,
  ...
  if (!failed && status_ == kCandidate) {
    ...
    LOG(INFO, "InsNodeImpl::VoteCallback[%ld], result:%s", their_term,
        response_ptr->vote_granted() ? "true" : "false");
...
void InsNodeImpl::ReplicateLog(std::string follower_id) {
  ...
  while (!stop_ && status_ == kLeader) {
    ...
    bool ok = rpc_client_.SendRequest(stub, &InsNode_Stub::AppendEntries,
...
void InsNodeImpl::Get(::google::protobuf::RpcController* controller,
  ...
  if (members_.size() > 1 && (now_timestamp - heartbeat_read_timestamp_) > 1000 * FLAGS_elect_timeout_min) {
...
void InsNodeImpl::ForwardKeepAlive(
  ...
  for (auto it = others_.begin(); it != others_.end(); it++) {
    ...
    boost::function<void(const ::galaxy::ins::KeepAliveRequest*,
                         ::galaxy::ins::KeepAliveResponse*, bool, int)>
        callback;
