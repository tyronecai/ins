...
bool TPrinter::AddRow(const std::vector<string>& cols) {
  if (cols.size() != _cols) {
    std::cerr << "arg num error: " << cols.size() << " vs " << _cols
              << std::endl;
    return false;
  }
  Line line;
...
void TPrinter::Print(bool has_head) {
  ...
  for (size_t i = 0; i < _cols; ++i) {
    ...
    std::cout << "  " << std::setfill(' ') << std::setw(_col_width[i])
              << std::setiosflags(std::ios::left) << _table[0][i];
  }
  std::cout << std::endl;
  if (has_head) {
  ...
  for (size_t i = 1; i < _table.size(); ++i) {
    for (size_t j = 0; j < _cols; ++j) {
      std::cout << "  " << std::setfill(' ') << std::setw(_col_width[j])
                << std::setiosflags(std::ios::left) << _table[i][j];
    }
    std::cout << std::endl;
  }
}
...
string TPrinter::ToString(bool has_head) {
  ...
  for (size_t i = 0; i < _cols; ++i) {
    line_len += 2 + _col_width[i];
    ostr << "  " << std::setfill(' ') << std::setw(_col_width[i])
         << std::setiosflags(std::ios::left) << _table[0][i];
  }
  ostr << std::endl;
  if (has_head) {
  ...
  for (size_t i = 1; i < _table.size(); ++i) {
    for (size_t j = 0; j < _cols; ++j) {
      ostr << "  " << std::setfill(' ') << std::setw(_col_width[j])
           << std::setiosflags(std::ios::left) << _table[i][j];
    }
    ostr << std::endl;
  }
