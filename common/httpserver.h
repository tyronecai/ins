...
class HttpFileServer {
 public:
  HttpFileServer(std::string root_path, int port)
 ...
 private:
  class Session {
   public:
    Session(FILE* in_stream, FILE* out_stream)
    ...
    void ShowDir(int file_fd, const std::string& path, int root_len) {
      ...
      for (size_t i = 0; i < children.size(); i++) {
        if (parent.size() > 0) {
          out += "<li><a href=\"/" + parent + "/" + children[i] + "\">" +
                 children[i] + "</a></li>\n";
        } else {
          out += "<li><a href=\"/" + children[i] + "\">" + children[i] +
                 "</a></li>\n";
        }
      }
      out += "</ul>";
  ...
  void HandleClient(sockaddr_in /*client_addr*/, int client_fd) {
    char header[256] = {'\0'};
    char method[256] = {'\0'};
    char path[256] = {'\0'};
    char version[256] = {'\0'};
