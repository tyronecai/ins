...
bool GetNewLog(bool append) {
  ...
  snprintf(buf, 30, "%02d-%02d.%02d:%02d:%02d.%06d", t.tm_mon + 1, t.tm_mday,
           t.tm_hour, t.tm_min, t.tm_sec, static_cast<int>(tv.tv_usec));
  ...
  while (
      (g_log_count && static_cast<int64_t>(g_log_set.size()) > g_log_count) ||
...
class AsyncLogger {
 public:
  AsyncLogger() : jobs_(&mu_), done_(&mu_), stopped_(false), size_(0) {
  ...
  void Flush() {
    MutexLock lock(&mu_);
    buffer_queue_->push(
        std::make_pair(0, reinterpret_cast<std::string*>(NULL)));
    jobs_.Signal();
    done_.Wait();
  }
...
bool RecoverHistory(const char* path) {
  ...
  for (std::vector<std::string>::iterator it = loglist.begin();
       it != loglist.end(); ++it) {
    g_log_set.insert(*it);
  }
  while (
...
void Logv(int log_level, const char* format, va_list ap) {
  ...
  if (thread_id == 0) {
    ...
    tid_str_len = snprintf(tid_str, sizeof(tid_str), " %5d ",
                           static_cast<int32_t>(thread_id));
  }

  static const char level_char[] = {'V', 'D', 'I', 'W', 'F'};
...
void RpcLogHandler(sofa::pbrpc::LogLevel level, const char* filename, int line,
  ...
  static int level_map[] = {16, 12, 8, 4, 4, 3, 2};
  char buf[1024];
...
LogStream::~LogStream() { Log(level_, "%s", oss_.str().c_str()); }
