...
tar zxf snappy-1.1.4.tar.gz && cd snappy-1.1.4 && autoreconf -fvi &&  ./configure --prefix=$base_dir/depends && make -j4 && make install
rm -rf ../snappy-1.1.4
...
cd sofa-pbrpc && make -e BOOST_HEADER_DIR=/data/ins/depends/boost_1_61_0 PROTOBUF_DIR=/data/ins/depends SNAPPY_DIR=/data/ins/depends OPT="-O2 -Wno-unused-parameter" -j4 && make -e PREFIX=/data/ins/depends instal
rm -rf ../sofa-pbrpc
