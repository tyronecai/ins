...
void InsSDK::ParseFlagFromArgs(int argc, char* argv[],
  ...
  if (members->size() < 1) {
    LOG(ERROR) << "invalid cluster size";
    abort();
  }
}
...
InsSDK::InsSDK(const std::string& server_list) {  // sperated by comma
  std::vector<std::string> members;
  boost::split(members, server_list, boost::is_any_of(","),
               boost::token_compress_on);
  Init(members);
}

InsSDK::InsSDK(const std::vector<std::string>& members) { Init(members); }
...
void InsSDK::PrepareServerList(std::vector<std::string>& server_list) {
  ...
  std::copy(members_.begin(), members_.end(), std::back_inserter(server_list));
}
...
bool InsSDK::Put(const std::string& key, const std::string& value,
                 SDKError* error) {
  ...
  for (auto it = server_list.begin(); it != server_list.end(); it++) {
    ...
    if (response.success() || response.uuid_expired()) {
      ...
      if (response.uuid_expired()) {
        LOG(WARNING) << "uuid is expired before put: " << key;
        *error = kUnknownUser;
        {
          MutexLock lock(mu_);
...
bool InsSDK::Get(const std::string& key, std::string* value, SDKError* error) {
  ...
  for (it = server_list.begin(); it != server_list.end(); it++) {
    ...
    request.set_key(key);
    bool ok = rpc_client_->SendRequest(stub, &InsNode_Stub::Get, &request,
                                       &response, 2, 1);
    if (!ok) {
    ...
    if (response.success() || response.uuid_expired()) {
      ...
      *value = response.value();
      if (response.uuid_expired()) {
        LOG(WARNING) << "uuid is expired before get: " << key;
        *error = kUnknownUser;
        {
          MutexLock lock(mu_);
          loggin_expired_ = true;
...
bool InsSDK::ScanOnce(const std::string& start_key, const std::string& end_key,
                      std::vector<KVPair>* buffer, SDKError* error) {
  assert(buffer);
  std::string value;
  SDKError err_temp = kOK;
  if (error == NULL) {
  ...
  if (!Get(start_key, &value, error)) {  // avoid network partition problem
    LOG(ERROR) << "the leader may be unavilable";
    return false;
  }
  std::vector<std::string> server_list;
  PrepareServerList(server_list);
  ...
  for (it = server_list.begin(); it != server_list.end(); it++) {
    ...
    bool ok = rpc_client_->SendRequest(stub, &InsNode_Stub::Scan, &request,
                                       &response, 5, 1);
    if (!ok) {
      LOG(ERROR) << "failed to rpc " << server_id;
      continue;
    ...
    if (response.success() || response.uuid_expired()) {
      ...
      if (response.uuid_expired()) {
        LOG(WARNING) << "uuid is expired before scan: [" << start_key << ", "
                      << end_key << ")";
        *error = kUnknownUser;
        {
          MutexLock lock(mu_);
...
bool InsSDK::Delete(const std::string& key, SDKError* error) {
  ...
  for (it = server_list.begin(); it != server_list.end(); it++) {
    ...
    request.set_key(key);
    bool ok = rpc_client_->SendRequest(stub, &InsNode_Stub::Delete, &request,
                                       &response, 2, 1);
    if (!ok) {
      LOG(ERROR) << "failed to rpc " << server_id;
      continue;
    ...
    if (response.success() || response.uuid_expired()) {
      ...
      if (response.uuid_expired()) {
        LOG(WARNING) << "uuid is expired before delete: " << key;
        *error = kUnknownUser;
        {
          MutexLock lock(mu_);
          loggin_expired_ = true;
...
bool InsSDK::Watch(const std::string& key, WatchCallback user_callback,
                   void* context, SDKError* error) {
  ...
  if (*error != kOK && *error != kNoSuchKey) {
    LOG(ERROR) << "faild to issue a watch: " << key;
    return false;
  }
  bool key_exist = true;
  if (*error == kNoSuchKey) {
...
void InsSDK::KeepAliveTask() {
  ...
  for (it = server_list.begin(); it != server_list.end(); it++) {
    ...
    bool ok = rpc_client_->SendRequest(stub, &InsNode_Stub::KeepAlive, &request,
                                       &response, 2, 1);
    if (!ok) {
      LOG(ERROR) << "failed to rpc " << server_id;
      continue;
  ...
  if (session_expire) {
    if (cb != NULL) {  // session timeout callback
      LOG(INFO) << "call user callback of session timeout: "
                 << handle_session_timeout_;
      cb(cb_ctx);
    }
    MakeSessionID();
    LOG(INFO) << "create a new session: " << GetSessionID();
  }
...
void InsSDK::KeepWatchCallback(const galaxy::ins::WatchRequest* request,
  ...
  if (session_expired ||
    ...
    SDKError error = kOK;
    if (session_expired) {
      LOG(INFO) << "force trigger " << response->watch_key()
    ...
    if (cb) {
      {
        ...
        LOG(INFO) << "watch #" << watch_id
                   << " trigger, key: " << response->watch_key();
      }
      WatchParam param;
      param.context = cb_ctx;
      if (response->uuid_expired()) {
        LOG(WARNING) << "uuid is expired before watch: "
                      << response->watch_key();
        param.key = "";
        param.value = "";
        param.deleted = false;
  ...
  if (!response->canceled()) {  // retry, if not cancel
    ...
    LOG(INFO) << "watch redirect to " << server_id
               << ", key: " << request->key();
    galaxy::ins::InsNode_Stub* stub;
    rpc_client_->GetStub(server_id, &stub);
    std::unique_ptr<galaxy::ins::InsNode_Stub> stub_guard(stub);
    ...
    boost::function<void(const galaxy::ins::WatchRequest*,
                         galaxy::ins::WatchResponse*, bool, int)> callback;
    callback = boost::bind(&InsSDK::KeepWatchCallback, this, _1, _2, _3, _4,
                           server_id, watch_id);
    rpc_client_->AsyncRequest(stub, &InsNode_Stub::Watch, req, rsps, callback,
                              FLAGS_ins_watch_timeout, 1);  // 120s timeout
  } else {
    LOG(INFO) << "the previous watch #" << watch_id
               << " is canceled, key: " << request->key();
  }
}

void InsSDK::BackupWatchTask(const std::string& key,
                             const std::string& old_value, bool key_exist,
                             std::string session_id, int64_t watch_id) {
  LOG(INFO) << "issue backup watch on key: " << key;
...
void InsSDK::KeepWatchTask(const std::string& key, const std::string& old_value,
                           bool key_exist, std::string session_id,
                           int64_t watch_id) {
  {
    MutexLock lock(mu_);
    if (stop_) {
    ...
    if (pending_watches_.find(watch_id) == pending_watches_.end()) {
      LOG(INFO) << "expired watch id: " << watch_id;
      return;
    }
  }

  if (session_id != GetSessionID()) {
    LOG(INFO) << "expired watch on " << key;
    return;
  }
  ...
  LOG(INFO) << "try watch to " << server_id << ", key: " << key;
  galaxy::ins::InsNode_Stub* stub;
  rpc_client_->GetStub(server_id, &stub);
  std::unique_ptr<galaxy::ins::InsNode_Stub> stub_guard(stub);
  ...
  callback = boost::bind(&InsSDK::KeepWatchCallback, this, _1, _2, _3, _4,
                         server_id, watch_id);
  rpc_client_->AsyncRequest(stub, &InsNode_Stub::Watch, request, response,
                            callback, FLAGS_ins_watch_timeout, 1);
}
...
bool InsSDK::Lock(const std::string& key, SDKError* error) {
  ...
  while (!TryLock(key, error)) {
    ...
    LOG(INFO) << "try lock again on: " << key;
    ThisThread::Sleep(1000);
    {
      MutexLock lock(mu_);
      if (stop_) {
...
bool InsSDK::TryLock(const std::string& key, SDKError* error) {
  ...
  for (it = server_list.begin(); it != server_list.end(); it++) {
    ...
    request.set_key(key);
    request.set_session_id(GetSessionID());
    bool ok = rpc_client_->SendRequest(stub, &InsNode_Stub::Lock, &request,
                                       &response, 2, 1);
    if (!ok) {
      LOG(ERROR) << "failed to rpc " << server_id;
      continue;
    ...
    if (response.uuid_expired()) {
      ...
      LOG(WARNING) << "uuid is expired before lock: " << key;
      *error = kUnknownUser;
      return false;
    }
    if (response.success()) {
...
bool InsSDK::UnLock(const std::string& key, SDKError* error) {
  ...
  for (it = server_list.begin(); it != server_list.end(); it++) {
    ...
    request.set_key(key);
    request.set_session_id(GetSessionID());
    bool ok = rpc_client_->SendRequest(stub, &InsNode_Stub::UnLock, &request,
                                       &response, 2, 1);
    if (!ok) {
      LOG(ERROR) << "failed to rpc " << server_id;
      continue;
    ...
    if (response.success() || response.uuid_expired()) {
      ...
      if (response.uuid_expired()) {
        LOG(WARNING) << "uuid is expired before unlock: " << key;
        *error = kUnknownUser;
        {
          MutexLock lock(mu_);
          loggin_expired_ = true;
...
bool InsSDK::Login(const std::string& username, const std::string& password,
                   SDKError* error) {
  {
    MutexLock lock(mu_);
  ...
  for (it = server_list.begin(); it != server_list.end(); ++it) {
    ...
    bool ok = rpc_client_->SendRequest(stub, &InsNode_Stub::Login, &request,
                                       &response, 2, 1);
    if (!ok) {
      LOG(ERROR) << "failed to rpc " << server_id;
      continue;
        ...
        switch (response.status()) {
          case galaxy::ins::kOk:
            *error = kOK;
            ...
            loggin_expired_ = false;
            return true;
          case galaxy::ins::kUnknownUser:
            *error = kUnknownUser;
            return false;
          case galaxy::ins::kPasswordError:
...
bool InsSDK::Logout(SDKError* error) {
  ...
  for (it = server_list.begin(); it != server_list.end(); ++it) {
    ...
    request.set_uuid(logged_uuid_);
    bool ok = rpc_client_->SendRequest(stub, &InsNode_Stub::Logout, &request,
                                       &response, 2, 1);
    if (!ok) {
      LOG(ERROR) << "failed to rpc " << server_id;
      continue;
...
bool InsSDK::Register(const std::string& username, const std::string& password,
                      SDKError* error) {
  if (username.empty()) {
    *error = kUserExists;
    return false;
  ...
  for (it = server_list.begin(); it != server_list.end(); ++it) {
    ...
    request.set_username(username);
    request.set_passwd(HashPassword(password));
    bool ok = rpc_client_->SendRequest(stub, &InsNode_Stub::Register, &request,
                                       &response, 2, 1);
    if (!ok) {
      LOG(ERROR) << "failed to rpc " << server_id;
      continue;
...
bool InsSDK::CleanBinlog(const std::string& server_id, int64_t end_index,
                         SDKError* error) {
  SDKError err_temp = kOK;
  if (error == NULL) {
    error = &err_temp;
  ...
  bool ok = rpc_client_->SendRequest(stub, &InsNode_Stub::CleanBinlog, &request,
                                     &response, 2, 1);
  if (!ok) {
    *error = kTimeout;
    return false;
  ...
  if (ok && !response.success()) {
    *error = kCleanBinlogFail;
    LOG(ERROR) << "remove binlog at " << end_index << " is unsafe";
    return false;
  }
  return true;
}
...
void InsSDK::RegisterSessionTimeout(void (*handle_session_timeout)(void*),
                                    void* ctx) {
  assert(handle_session_timeout);
  MutexLock lock(mu_);
  handle_session_timeout_ = handle_session_timeout;
  session_timeout_ctx_ = ctx;
  LOG(INFO) << "session timeout handler: " << handle_session_timeout_;
}
...
bool ScanResult::Done() { return buffer_.empty(); }

SDKError ScanResult::Error() { return error_; }
