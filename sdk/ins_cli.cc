  ...
  do {
    if (FLAGS_ins_cmd == "show") {
      ...
      for (it = cluster_info.begin(); it != cluster_info.end(); it++) {
        ...
        cprinter.AddRow(
            7, it->server_id.c_str(), s_status.c_str(),
      ...
      sprinter.AddRow(11, "server id", "status", "kind", "Put", "Get", "Delete",
                      "Scan", "KeepAlive", "Lock", "Unlock", "Watch");
      ...
      for (std::vector<NodeStatInfo>::iterator it = stat_info.begin();
        ...
        sprinter.AddRow(
            11, it->server_id.c_str(), s_status.c_str(), "current",
      ...
      printf(
          "not a valid command for logged user, "
