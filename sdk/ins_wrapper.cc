...
// ----- InsSDK Wrappers -----
InsSDK* GetSDK(const char* server_list) { return new InsSDK(server_list); }
...
bool SDKPut(InsSDK* sdk, const char* key, int key_len, const char* buf,
            int buf_len, SDKError* error) {
  if (sdk == NULL) {
...
bool SDKGet(InsSDK* sdk, const char* key, int key_len, char** buf_ptr,
            int32_t* buf_len, SDKError* error) {
  if (sdk == NULL) {
    *error = kPermissionDenied;
    return false;
  }
...
ScanResult* SDKScan(InsSDK* sdk, const char* start_key, int start_key_len,
  ...
  return sdk->Scan(std::string(start_key, start_key_len),
                   std::string(end_key, end_key_len));
}
...
bool SDKWatch(InsSDK* sdk, const char* key, int key_len, CWatchCallback wrapper,
  ...
  pack->ctx = context;
  return sdk->Watch(std::string(key, key_len), WatchCallbackWrapper, pack,
                    error);
}
...
bool SDKLogin(InsSDK* sdk, const char* username, const char* password,
              SDKError* error) {
  if (sdk == NULL) {
    *error = kPermissionDenied;
    return false;
...
bool SDKRegister(InsSDK* sdk, const char* username, const char* password,
                 SDKError* error) {
  if (sdk == NULL) {
    *error = kPermissionDenied;
    return false;
...
const char* SDKGetSessionID(InsSDK* sdk) {
  static char id_buf[64] = {0};
  if (sdk == NULL) {
    return "";
  }
  strncpy(id_buf, sdk->GetSessionID().c_str(), 63);
  return id_buf;
}
...
const char* SDKGetCurrentUserID(InsSDK* sdk) {
  ...
  strncpy(id_buf, sdk->GetCurrentUserID().c_str(), 63);
  return id_buf;
}
...
void SDKRegisterSessionTimeout(InsSDK* sdk,
                               SessionTimeoutCallback handle_session_timeout,
                               long callback_id, void* ctx) {
  if (sdk == NULL) {
    return;
  }
  CallbackPack* pack = new CallbackPack();
  pack->callback_wrapper =
      reinterpret_cast<AbstractFunc>(handle_session_timeout);
  pack->callback_id = callback_id;
  pack->ctx = ctx;
